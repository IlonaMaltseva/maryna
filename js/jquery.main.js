...
jQuery(function () {
	...
	isElementExist("#modal", initModal);
	isElementExist(".project-slider", initReviewSlider);
...
function initForm() {
	const formFields = $('.form-field input, .form-field textarea');

	formFields.on('focus click', function () {
...
function initModal() {
	$("#modal").iziModal({
		...
		bodyOverflow: true,
		radius: 0,
		overlayColor: 'rgba(25, 51, 70, 0.7)',
	});
}
...
function initSmartMenu() {
	var distanceBetweenMenuAndNav = $(".header__container").offset().top + $(".header__container").innerHeight() - $(".nav").offset().top - $(".nav").innerHeight();
	$(".menu").smartmenus({
	...
	$('.menu').children().last().addClass('nav-sm-reverse');
	...
	$.extend($.SmartMenus, {
		...
		prototype: {
			...
			disable: function(noOverlay) {
				if (!this.disabled) {
					this.menuHideAll();
					// display bg-overlay over the menu to prevent interaction
			...
			winResize: function(e) {
				if (!this.handleEvents()) {
					// we still need to resize the disable bg-overlay if it's visible
					if (this.$disableOverlay) {
						var pos = this.$root.offset();
